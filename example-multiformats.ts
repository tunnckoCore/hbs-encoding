import * as Block from "multiformats/block";
import * as codec from "@ipld/dag-cbor";
import { sha256 as hasher } from "multiformats/hashes/sha2";
import { encodeHbs } from "./src";

const fixture = {
  external_app_url: null,
  id: "8711",
  image_url:
    "data:image/svg+xml;base64,",
  is_unique: true,
  media_type: "image",
  // metadata: {
  //   attributes: [
  //     { trait_type: "Background", value: "Classic Punks BG" },
  //     { trait_type: "Type", value: "Human Melanin Level Goth" },
  //     { trait_type: "Cloths", value: "Vampire Attack Attire" },
  //     { trait_type: "Head", value: "Blockthink Receiver" },
  //     { trait_type: "Eyes", value: "Visoor Pink" },
  //     { trait_type: "Classification", value: "Comrade" },
  //     { trait_type: "Affiliation", value: "Corrupted" },
  //     { trait_type: "Rank", value: "4300" },
  //   ],
  //   ethscription_id: "0xa51759f47d949e755de3c30964e0fb14d7b638a12354fd40fc62eba0313e2653",
  //   ethscription_number: 6169177,
  //   name: "Comrade #8712",
  // },
  owner: "0x4212D149F77308a87ce9928F1095eDdb894f4D68",
  animation_url: "https://example.com/foo.gif",
  thumbnails: null,
  token: "0xBB41E24dA83DcAb001bd085879c66cFCB4eED522",
};

const block = await Block.encode({
  value: fixture,
  codec,
  hasher,
});

const decoded = await Block.decode({
  bytes: block.bytes,
  codec,
  hasher,
});

// console.log(block.cid.toString());
// // console.log(decoded.value);
// console.log("raw json size:", new TextEncoder().encode(JSON.stringify(fixture)).byteLength);
// console.log(
//   "multiformats size:",
//   (block.bytes.byteLength ?? block.bytes.length) + block.cid.toString().length,
// );
// console.log(new TextDecoder().decode(block.bytes));

// console.log("hbs2 size:", encodeHbs(fixture).length);
